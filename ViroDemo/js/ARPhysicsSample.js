 ...
 */
export default class ARPhysicsSample extends Component {
  constructor() {
    super();
    ...
    return (state, position, source) => {
      this.sceneRef.getCameraOrientationAsync().then(camTransform => {
        this.ball.getTransformAsync().then(ballTransform => {
          const ballPos = ballTransform.position;
          ...
          const pullStrength = 5; // Force multiplier.
          const pullStrengthVec = [
  ...
  _renderCubes() {
    const views = [];
    for (const i = 0; i < this.state.totalCubes; i++) {
      const cubeKey = "CubeTag_" + i;
      views.push(
        <ViroBox
          scale={[0.2, 0.2, 0.2]}
          position={[-0.5, 1, -1.3]}
